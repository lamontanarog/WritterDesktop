...
import {
  Box, Button, Container, Typography, Paper, CircularProgress, Alert, Avatar, Chip, Dialog, DialogTitle, DialogContent, DialogActions, TextField
} from "@mui/material";
import { ArrowBack, AccessTime as AccessTimeIcon, TextFields as TextFieldsIcon } from "@mui/icons-material";
import { toast } from "sonner";

import { useGetCurrentUserQuery, useGetTextsQuery, useDeleteTextMutation, useUpdateTextMutation, useGetIdeasQuery } from "../features/api/apiSlice";
...
const Profile = () => {
  ...
  const { data: user, isLoading: isLoadingUser, isError: isUserError } = useGetCurrentUserQuery(undefined, { skip: !localStorage.getItem("token") });
  const { data: textsResponse, isLoading: isLoadingTexts, isError: isTextsError, refetch } = useGetTextsQuery({ page, limit: 10 }, { skip: !localStorage.getItem("token") });
  const texts = textsResponse?.data || [];

  // Fetch de ideas
  const { data: ideasResponse } = useGetIdeasQuery({ page: 1, limit: 100 }, { skip: texts.length === 0 });
  ...
  if (isLoadingUser || isLoadingTexts) return <Box display="flex" justifyContent="center" alignItems="center" minHeight="100vh"><CircularProgress /></Box>;
  if (isUserError || !user) return <Alert severity="error">Error al cargar el perfil</Alert>;

  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Button onClick={() => navigate("/home", { replace: true })} variant="contained" startIcon={<ArrowBack />} sx={{ mb: 4 }}>Volver al inicio</Button>
      ...
      {isTextsError ? (
        ...
        texts.map((text) => (
          <TextItem
            ...
            onView={() => { setSelectedText(text); setOpenDialog(true); }}
            onEdit={() => { setSelectedText(text); setEditContent(text.content); setOpenEditDialog(true); }}
            onDelete={() => { setSelectedText(text); setOpenDeleteDialog(true); }}
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          ...
          <Box display="flex" gap={1} mt={3}>
            ...
            <Chip
              icon={<TextFieldsIcon />}
              label={`${selectedText?.content.trim().split(/\s+/).length} palabras`}
              size="small"
              variant="outlined"
              sx={{ borderRadius: 1 }}
            />
